...
def dashboard():
    ...
    return render_template_string(
        """
        <!doctype html><html><head><meta charset="utf-8"><title>TIMEŒ Airbnb Ops</title>
        <style>body{font-family:system-ui;max-width:1000px;margin:40px auto;padding:0 20px}.grid{display:grid;grid-template-columns:repeat(3,1fr);gap:12px}.card{padding:18px;border:1px solid #ddd;border-radius:12px}.alert{padding:10px;background:#fff3cd;margin:8px 0;border-radius:8px}.bad{background:#f8d7da}</style></head>
        ...
        <h2>Operational alerts</h2>{% for alert in alerts %}<div class="alert">{{ alert }}</div>{% else %}<p>No alerts.</p>{% endfor %}
        <h2>Open work</h2><p>{{ tasks|length }} task(s), {{ low_inventory|length }} low-inventory item(s), {{ maintenance|length }} maintenance issue(s).</p>
        </body></html>
        """, summary=summary, alerts=alerts, tasks=tasks, low_inventory=low_inventory, maintenance=maintenance, money=money,
    )
...
def create_booking():
    ...
    try:
        booking = Booking(
            ...
            nightly_rate=Decimal(str(data.get("nightly_rate", _config().nightly_rate))),
...
def create_expense():
    ...
    try:
        expense = Expense(
            expense_id=str(data["expense_id"]), category=str(data["category"]), amount=Decimal(str(data["amount"])),
            expense_date=date.fromisoformat(data.get("expense_date", date.today().isoformat())), note=str(data.get("note", "")),
        )
