"""Safe notification queue for Airbnb operational tasks.

This module creates deterministic notification records only. It does not send
messages or call external providers.
"""
...
def build_notification_queue(tasks: list[OpsTask], today: date) -> list[Notification]:
    """Create notifications for tasks due today or overdue, without sending."""
    ...
    for task in tasks:
        ...
        channels = (
            ("owner_dashboard", "manager")
            if task.task_type in {"turnover", "check_in"}
