...
<style>body{font-family:system-ui;background:#0b1020;color:#eef;padding:24px;max-width:1100px;margin:auto}.metrics{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:18px}.metric,.card{background:#151d32;border:1px solid #293554;border-radius:14px;padding:16px}.metric b{display:block;font-size:1.35rem;margin-top:6px}.grid{display:grid;grid-template-columns:repeat(3,1fr);gap:14px}.escalate{border-left:5px solid #ff5d6c}.action{border-left:5px solid #ffb84d}.monitor{border-left:5px solid #62d7a3}small{color:#9ba8c7}@media(max-width:800px){.metrics,.grid{grid-template-columns:1fr 1fr}}@media(max-width:520px){.metrics,.grid{grid-template-columns:1fr}}</style>
...
def dashboard():
    ...
    return render_template_string(_TEMPLATE, center=center, live=live, **metrics)
